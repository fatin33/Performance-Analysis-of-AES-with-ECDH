...
def run_comparison():
    ...
    try:
        ...
        highlight_color_total_size = "#fff3e0"    # Light orange
        highlight_color_cpu = "#ffebee"           # Light red
        highlight_color_memory = "#f3e5f5"        # Light purple
        highlight_color_security = "#e8f5e9"      # Light green
        
        for i, (label, value) in enumerate(luc_data):
            # Determine background color based on label
            bg_color = "#f1f8e9"  # Default light green
        ...
        try:
            ...
            for i, (label, value) in enumerate(ecdh_data):
                # Determine background color based on label
                bg_color = "#e8eaf6"  # Default light blue
        ...
        # Add legend for color highlights
        legend_frame = tk.Frame(results_frame, bg="white")
        legend_frame.grid(row=row+len(luc_data)+1, column=0, columnspan=4, sticky="ew", padx=10, pady=10)
        
        tk.Label(legend_frame, text="Highlight Legend:", font=("Arial", 10, "bold"), 
        ...
        legend_colors = [
            (highlight_color_total_setup, "Total Setup Time"),
            (highlight_color_total_size, "Total Key Size"),
            (highlight_color_cpu, "CPU Usage"),
            (highlight_color_memory, "Memory Usage"),
            (highlight_color_security, "Security Level")
        ]
        
        for color, text in legend_colors:
            item_frame = tk.Frame(legend_items, bg="white")
            item_frame.pack(side="left", padx=10)
            tk.Label(item_frame, text="", bg=color, width=3, height=1).pack(side="left", padx=(0,5))
            tk.Label(item_frame, text=text, font=("Arial", 9), bg="white").pack(side="left")
